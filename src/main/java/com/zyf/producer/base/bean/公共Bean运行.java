...
@Slf4j
public class 公共Bean运行<T extends BaseBeanContext, S extends 公共Bean生产者<T>> {
    ...
    public void 执行(String[] args, Supplier<S> producerSupplier, Supplier<T> beanSupplier, Consumer<Bean执行方式<T>> consumer) throws InterruptedException {
        loadConfig();
    ...
    private void 最终执行(Supplier<S> producerSupplier, Supplier<T> beanSupplier, Consumer<Bean执行方式<T>> consumer) throws InterruptedException {
        ...
        try {
            ...
            final RingBuffer<T> ringBuffer = RingBufferUtil.getRingBuffer(4096,
                    consumerPool,
                    beanSupplier,
                    consumer,
                    disruptor -> {
    ...
    public static <T extends 公共Bean消费者> T[] 创建消费者数组(int size, AtomicInteger counter, Supplier<T> supplier) {
