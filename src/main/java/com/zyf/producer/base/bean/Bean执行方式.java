...
public class Bean执行方式<T extends BaseBeanContext> {
    ...
    public Bean执行方式(Disruptor<T> disruptor) {
        this.disruptor = disruptor;
    }


    public 串联重复消费下一步执行方式<T> 串联重复消费(公共Bean消费者<T>... handlers) {
        total += handlers.length;
        return new 串联重复消费下一步执行方式<T>(disruptor.handleEventsWith(handlers));
    }

    public 串联重复多次下一步执行方式<T> 串联重复消费(List<公共Bean消费者<T>[]> handlerList) {
        List<EventHandlerGroup<T>> list = new ArrayList<>();
        for (公共Bean消费者<T>[] eventHandlerAttr : handlerList) {
    ...
    public 并发重复多次下一步执行方式<T> 并发消费(List<公共Bean消费者<T>[]> handlerList) {
        Set<公共Bean消费者<T>[]> handlerSet = new HashSet<>(handlerList);
        if (handlerSet.size() != handlerList.size()) {
            throw new RuntimeException("不允许数组重复add");
        }

        List<EventHandlerGroup<T>> list = new ArrayList<>();
        for (公共Bean消费者<T>[] workHandlerAttr : handlerList) {
    ...
    public class 串联重复消费下一步执行方式<T extends BaseBeanContext> {
    ...
    public class 串联重复多次下一步执行方式<T extends BaseBeanContext> {
        ...
        public 串联重复多次下一步执行方式<T> 串联重复消费(公共Bean消费者<T>... handlers) {
            total += handlers.length;
            List<EventHandlerGroup<T>> list = new ArrayList<>();
    ...
    public class 并发重复多次下一步执行方式<T extends BaseBeanContext> {
        private List<EventHandlerGroup<T>> eventHandlerGroups;
        ...
        public 并发重复多次下一步执行方式<T> 并发消费(List<公共Bean消费者<T>[]> handlerList) {
            ...
            for (公共Bean消费者<T>[] handlers : handlerList) {
                total += handlers.length;
